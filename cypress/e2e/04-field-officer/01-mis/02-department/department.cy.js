// npm run auto
// npm run dev
// npx cypress run --spec cypress/e2e/04-field-officer/01-mis/02-department/department.cy.js --browser chrome --headed
...
describe("02. Department Module Test Cases", function () {
  ...
  it("02.05. Verify that the department forms edit mode can be toggled on and off all fields should be disabled or return to view-only mode.", function () {
    grid_department.turnOnEditMode();
  });

  it("02.06. Verify the department edit reset button functions correctly.", function () {
    grid_department.editResetButton();
  });

  it("02.07. Verify the department submit button functions correctly.", function () {
  ...
  it("02.09. Verify the user can filter inactive departments using the status dropdown on the department list page.", function () {
    grid_department.statusInactiveDropdownCheck();
  });

  it("02.10. Verify the active result is filtered correctly by the status dropdown.", function () {
    grid_department.statusActiveDropdownCheck();
  });

  it("02.11. Verify the search returns correct results when searching by department name.", function () {
    grid_department.searchInDepartmentName();
  });

  it("02.12. Verify the grid list resets successfully when the reset button is clicked on the department page.", function () {
    grid_department.gridResetButtonCheck();
  });

  it("02.13. Verify that the grid list is refreshed successfully when the refresh button is clicked on the department page.", function () {
    grid_department.gridRefreshButtonCheck();
  });

  it("02.14. Verify that when the user clicks the Search button on the department grid page, the system redirects to the department List page..", function () {
